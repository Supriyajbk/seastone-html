...
window.Sticksy = (function () {
    ...
    if (this._props.listen) {
      var mutationObserver = new MutationObserver(function (mutations) {
        ...
        mutations.forEach(function (mutation) {
          if (
            (that._dummyNodes.indexOf(mutation.target) !== -1 &&
              mutation.attributeName === "style") ||
            (that._stickyNodes.indexOf(mutation.target) !== -1 &&
              mutation.attributeName === "style")
          ) {
            return;
          }
  ...
  Constructor.prototype._enableElements = function (elements) {
    for (var i = 0; i < elements.length; i++) {
      elements[i].style.display = getComputedStyle(
        this._stickyNodes[i]
      ).display;
    }
  };
  ...
  var Utils = {
    ...
    getComputedBox: function (elem) {
      ...
      return {
        ...
        topWithMargin:
          window.pageYOffset + box.top - Utils.parseNumber(style.marginTop),
        bottomWithMargin:
          window.pageYOffset +
          box.bottom +
          Utils.parseNumber(style.marginBottom),
      };
    },
  };
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  let $isleftSticky = document.querySelector(".expertise-left-stck");
  let instance;

  let $mobile = window.matchMedia("(max-width: 767px)");
  let $ipad = window.matchMedia("(max-width: 1023px)");

  function handleStickyBehavior() {
    if (!$mobile.matches && $isSticky) {
      if (!instance) {
        instance = new Sticksy($isSticky, {
  ...
  $mobile.addEventListener("change", handleStickyBehavior);
  window.addEventListener("resize", handleStickyBehavior);

  let stickOnDesktop = () => {
    if (!$ipad.matches && $isleftSticky) {
      if (!instance) {
        instance = new Sticksy($isleftSticky, {
          topSpacing: $headerHeight + 80, //80px space from padding in css-- so added here
          listen: true,
        });
      } else {
        instance.refresh();
  ...
  stickOnDesktop();
  $mobile.addEventListener("change", stickOnDesktop);
  window.addEventListener("resize", stickOnDesktop);
});
